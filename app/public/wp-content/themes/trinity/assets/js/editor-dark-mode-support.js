/* global trinityIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'trinity-supports-dark-theme' ) ) {
	trinityDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function trinityDarkModeEditorInit( attempt ) {
	...
	if ( trinityIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						trinityDarkModeEditorInit( attempt + 1 );
					},
